...
def generate_base64_graph(graph_type):
    ...
    if graph_type=="bar":
        ...
        x_candidates = path.select_dtypes(include=['object', 'datetime']).columns.tolist()
        # y軸の候補（数値データ）
        y_candidates = path.select_dtypes(include=['number']).columns.tolist()
        
        x_axis = x_candidates[0] if x_candidates else None
        y_axis = y_candidates if y_candidates else None

        ax = path.plot(x=x_axis, y=y_axis, kind='bar', figsize=(10, 6))
        ...
        ax = path.plot(kind='line', figsize=(10, 6))
        ...
        numeric_columns = path.select_dtypes(include=['number']).columns.tolist()
        column_to_plot = numeric_columns[0]
        data_to_plot = path[column_to_plot].dropna()
        ...
        numeric_candidates = path.select_dtypes(include=['number']).columns.tolist()
        ax = path[numeric_candidates].plot(kind='box', figsize=(10, 6))
        ...
        numeric_candidates = path.select_dtypes(include=['number']).columns.tolist()
        ax = path[numeric_candidates].plot(kind='area', figsize=(10, 6), stacked=False)  # stacked=False で積み上げない面積グラフ
        ...
        category_columns = path.select_dtypes(include=['object', 'category']).columns.tolist()
        numeric_columns = path.select_dtypes(include=['number']).columns.tolist()
        ...
        data = path.groupby(category_column)[value_column].sum()
        total = data.sum()
        ...
        fig, ax = plt.subplots(figsize=(10, 6))
        ax.scatter(path[x_column], path[y_column])
